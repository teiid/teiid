...
public class MMStatement extends WrapperImpl implements Statement {
    ...
    protected ClientSideDQP getDQP() {
    	return this.driverConnection.dqp;
    }
    ...
    protected TimeZone getServerTimeZone() {
    	return this.driverConnection.serverConn.getLogonResult().getTimeZone();
    }
    ...
    public void close() throws SQLException {
        ...
        if(currentRequestID > -1) {
            if(resultSet == null) {
            	try {
					this.driverConnection.dqp.closeRequest(currentRequestID);
    ...
    private ResultsMessage executeAsynchAndWait(RequestMessage reqMsg, long timeout) throws SQLException, InterruptedException, TimeoutException, CommunicationException {
		...
		try {
			pendingResult = this.driverConnection.dqp.executeRequest(this.currentRequestID, reqMsg);
    ...
    protected void cancelRequest() throws SQLException {
        ...
        try {
			this.driverConnection.dqp.cancelRequest(currentRequestID);
		} catch (MetaMatrixProcessingException e) {
			throw MMSQLException.create(e);
		} catch (MetaMatrixComponentException e) {
			throw MMSQLException.create(e);
