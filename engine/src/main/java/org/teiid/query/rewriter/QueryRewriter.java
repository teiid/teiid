...
/**
 * Rewrites commands and command fragments to a form that is better for planning
 * and execution. There is a current limitation that command objects themselves
 * cannot change type, since the same object is always used.
 */
public class QueryRewriter {

	private static final Constant ZERO_CONSTANT = new Constant(0, DataTypeManager.DefaultDataClasses.INTEGER);
	public static final CompareCriteria TRUE_CRITERIA = new ImmutableCompareCriteria(
			new Constant(1, DataTypeManager.DefaultDataClasses.INTEGER), CompareCriteria.EQ,
			new Constant(1, DataTypeManager.DefaultDataClasses.INTEGER));
	public static final CompareCriteria FALSE_CRITERIA = new ImmutableCompareCriteria(
			new Constant(1, DataTypeManager.DefaultDataClasses.INTEGER), CompareCriteria.EQ, ZERO_CONSTANT);
	public static final CompareCriteria UNKNOWN_CRITERIA = new ImmutableCompareCriteria(
			new Constant(null, DataTypeManager.DefaultDataClasses.STRING), CompareCriteria.NE,
			new Constant(null, DataTypeManager.DefaultDataClasses.STRING));

	private static final Map<String, String> ALIASED_FUNCTIONS = new TreeMap<String, String>(
	...
	public static Command evaluateAndRewrite(Command command, Evaluator eval, CommandContext context,
	...
	/**
	 * Rewrites the command and all of its subcommands (both embedded and
	 * non-embedded)
	 * 
	 * @param command
	 * @param removeOrderBy
	 ...
	 */
	private Command rewriteCommand(Command command, boolean removeOrderBy)
			throws TeiidComponentException, TeiidProcessingException {
			...
			if (queryCommand.getWith() != null) {
				outer: for (WithQueryCommand withQueryCommand : queryCommand.getWith()) {
					if (withQueryCommand.getColumns() == null) {
						List<ElementSymbol> columns = ResolverUtil
								.resolveElementsInGroup(withQueryCommand.getGroupSymbol(), metadata);
					...
					if (GroupCollectorVisitor.getGroups(withQueryCommand.getCommand(), false).isEmpty()) {
						...
						for (Expression ex : FunctionCollectorVisitor.getFunctions(withQueryCommand.getCommand(), false,
								true)) {
	...
	private List<Statement> rewriteStatements(List<Statement> statements) throws TeiidComponentException {
		...
		while (stmtIter.hasNext()) {
			...
			try {
				...
				/*
				 * defer the processing of the exception until runtime as there may be an
				 * exception handler
				 */
	 ...
	 * @throws QueryValidatorException
	 */
	private void rewriteSubqueryContainer(SubqueryContainer container, boolean removeOrderBy)
			throws TeiidComponentException, TeiidProcessingException {
		if (rewriteSubcommands && container.getCommand() != null
				&& (container.getCommand().getProcessorPlan() == null || processing)) {
	...
	private Command rewriteQuery(Query query, Map<Object, WithQueryCommand> replacements)
		...
		if (crit != null) {
			if (query.getIsXML()) {
				// only rewrite expressions, removing whole predicates could change the nature
				// of the query,
				// because we aren't considering the xml pseudo-functions context, row limit,
				// etc.
		...
		if (from != null && !query.getIsXML()) {
			...
			for (int i = 0; i < symbols.size(); i++) {
				...
				plannedResult.reset();
				rmc.findSubquery(SymbolMap.getExpression(symbol),
						context != null ? context.getOptions().isSubqueryUnnestDefault() : false, plannedResult);
				...
				symbols.set(i, new AliasSymbol(ExpressionSymbol.getName(symbol),
						(Expression) q.getProjectedSymbols().get(0).clone()));
			}
		}
	...
	private Query convertToJoin(PlannedResult plannedResult, Set<String> names, Query query, boolean leftOuter)
		...
		for (int i = 0; i < plannedResult.leftExpressions.size(); i++) {
			plannedResult.nonEquiJoinCriteria
					.add(new CompareCriteria(SymbolMap.getExpression((Expression) plannedResult.leftExpressions.get(i)),
	...
	private void determineCorrelatedReferences(List<GroupSymbol> groups, PlannedResult plannedResult) {
		if (plannedResult.query.getCorrelatedReferences() == null) {
			...
			// there is a little bit of a design problem here that null usually means no
			// refs.
	...
	/**
	 * Rewrite the order by clause. Unrelated order by expressions will cause the
	 * creation of nested inline views.
	 * 
	 * @param query
	...
	public static void rewriteOrderBy(QueryCommand queryCommand, final OrderBy orderBy, final List projectedSymbols,
			CommandContext context, QueryMetadataInterface metadata)
			throws TeiidComponentException, TeiidProcessingException {
		...
		for (int i = 0; i < orderBy.getVariableCount(); i++) {
			...
			if (!previousExpressions.add(expr)
					|| (queryCommand instanceof Query && EvaluatableVisitor.willBecomeConstant(expr))) {
	 ...
	 * column name in the target table. This ensures that they will all be uniquely
	 * named.
	 * 
	 * @param query
	 * @throws QueryValidatorException
	...
	private Command rewriteSelectInto(Query query) throws TeiidProcessingException {
		...
		try {
			List<ElementSymbol> allIntoElements = Util
					.deepClone(ResolverUtil.resolveElementsInGroup(into.getGroup(), metadata), ElementSymbol.class);
	...
	private Insert correctDatatypes(Insert insert) {
		...
		if (needsView) {
			try {
				insert.setQueryExpression(createInlineViewQuery(new GroupSymbol("X"), insert.getQueryExpression(), //$NON-NLS-1$
						metadata, insert.getVariables()));
			} catch (TeiidException err) {
	...
	private JoinPredicate rewriteJoinPredicate(Query parent, JoinPredicate predicate,
			Map<Object, WithQueryCommand> replacements) throws TeiidComponentException, TeiidProcessingException {
		...
		if (joinCrits != null && joinCrits.size() > 0) {
			...
			if (criteria instanceof CompoundCriteria
					&& ((CompoundCriteria) criteria).getOperator() == CompoundCriteria.AND) {
	 ...
	 * 
	 * @param criteria
	 *            The criteria to rewrite
	 * @param metadata
	 * @param userCriteria
	 *            The criteria on user's command, used in rewriting HasCriteria in
		...
		if (criteria instanceof CompoundCriteria) {
			...
			if (exists.getCommand().getProcessorPlan() == null) {
				if (exists.getCommand() instanceof Query) {
					...
					if ((query.getLimit() == null || query.getOrderBy() == null)
							&& query.getSelect().getProjectedSymbols().size() > 1) {
			...
			return rewriteCriteria(new CompareCriteria(((ExpressionCriteria) criteria).getExpression(),
	...
	private void rewriteWithExplicitArray(Expression ex, SubqueryContainer<QueryCommand> sub)
		...
		Query query = QueryRewriter.createInlineViewQuery(new GroupSymbol("x"), sub.getCommand(), metadata, //$NON-NLS-1$
				sub.getCommand().getProjectedSymbols());
	...
	/**
	 * May be simplified if this is an AND and a sub criteria is always false or if
	 * this is an OR and a sub criteria is always true
	 */
	private Criteria rewriteCriteria(CompoundCriteria criteria, boolean rewrite)
			throws TeiidComponentException, TeiidProcessingException {
						...
						if (converted instanceof SetCriteria) {
							...
							if (cc.getOperator() == CompareCriteria.EQ && cc.getRightExpression() instanceof Constant) {
								...
								if (crit == null) {
									...
									SetCriteria sc = new SetCriteria(cc.getLeftExpression(),
											DataTypeManager.isHashable(other.getRightExpression().getType())
													? new LinkedHashSet<Constant>()
													: new TreeSet<Constant>());
	 ...
	 */
	private Criteria rewriteAndConjunct(Criteria converted, Map<Expression, Criteria> exprMap,
			LinkedHashSet<Criteria> newCrits) {
			...
			if (!sc.isNegated() && sc.isAllConstants()) {
				if (crit == null) {
					...
					for (Iterator<Constant> exprIter = sc.getValues().iterator(); exprIter.hasNext();) {
						if (!Evaluator.compare(cc.getOperator(), exprIter.next().getValue(),
				...
				if (crit == null) {
					...
					if (cc.getOperator() == CompareCriteria.EQ) {
						...
						if (sc.getNumberOfValues() == 1) {
							CompareCriteria comp = new CompareCriteria(sc.getExpression(), CompareCriteria.EQ,
									(Expression) sc.getValues().iterator().next());
							newCrits.add(comp);
					...
					if (cc1.getOperator() == CompareCriteria.NE) {
						...
						if (!Evaluator.compare(cc.getOperator(), ((Constant) cc1.getRightExpression()).getValue(),
								((Constant) cc.getRightExpression()).getValue())) {
							return FALSE_CRITERIA;
						}
						return null;
					...
					if (cc.getOperator() == CompareCriteria.EQ) {
						if (!Evaluator.compare(cc1.getOperator(), ((Constant) cc.getRightExpression()).getValue(),
	...
	private Criteria evaluateCriteria(Criteria crit) throws TeiidComponentException, TeiidProcessingException {
		if (EvaluatableVisitor.isFullyEvaluatable(crit, !processing)) {
			try {
				...
				throw new QueryValidatorException(QueryPlugin.Event.TEIID30372, e,
						QueryPlugin.Util.gs(QueryPlugin.Event.TEIID30372, crit));
			}
		}
	...
	private Criteria rewriteCriteria(NotCriteria criteria) throws TeiidComponentException, TeiidProcessingException {
		...
		if (innerCrit instanceof CompoundCriteria) {
			// reduce to only negation of predicates, so that the null/unknown handling
			// criteria is applied appropriately
	...
	/**
	 * Rewrites "a [NOT] BETWEEN b AND c" as "a &gt;= b AND a &lt;= c", or as "a
	 * &lt;= b OR a&gt;= c"
	 * 
	 * @param criteria
	 * @return
	 * @throws QueryValidatorException
	 */
	private Criteria rewriteCriteria(BetweenCriteria criteria)
			throws TeiidComponentException, TeiidProcessingException {
		CompareCriteria lowerCriteria = new CompareCriteria(criteria.getExpression(),
				criteria.isNegated() ? CompareCriteria.LT : CompareCriteria.GE, criteria.getLowerExpression());
		CompareCriteria upperCriteria = new CompareCriteria(criteria.getExpression(),
				criteria.isNegated() ? CompareCriteria.GT : CompareCriteria.LE, criteria.getUpperExpression());
		CompoundCriteria newCriteria = new CompoundCriteria(
				criteria.isNegated() ? CompoundCriteria.OR : CompoundCriteria.AND, lowerCriteria, upperCriteria);

		return rewriteCriteria(newCriteria);
	}

	private Criteria rewriteCriteria(CompareCriteria criteria)
			throws TeiidComponentException, TeiidProcessingException {
		...
		if (EvaluatableVisitor.willBecomeConstant(rightExpr)) {
			...
			case CompareCriteria.LT:
				criteria.setOperator(CompareCriteria.GT);
				break;
			case CompareCriteria.LE:
				criteria.setOperator(CompareCriteria.GE);
				break;
			case CompareCriteria.GT:
				criteria.setOperator(CompareCriteria.LT);
				break;
			case CompareCriteria.GE:
				criteria.setOperator(CompareCriteria.LE);
				break;
			}
			rightConstant = true;
		}
	 ...
	 */
	private Criteria rewriteCriteria(SubqueryCompareCriteria criteria)
			throws TeiidComponentException, TeiidProcessingException {
		...
		if (criteria.getCommand() != null && criteria.getCommand().getProcessorPlan() == null) {
			...
			if (criteria.getPredicateQuantifier() != SubqueryCompareCriteria.ALL
					&& criteria.getOperator() != CompareCriteria.EQ && criteria.getOperator() != CompareCriteria.NE) {
				...
				if (useView) {
					Query q = createInlineViewQuery(new GroupSymbol("X"), criteria.getCommand(), metadata, //$NON-NLS-1$
							criteria.getCommand().getProjectedSymbols());
		...
		if (criteria.getCommand() != null && !RelationalNodeUtil.shouldExecute(criteria.getCommand(), false, true)) {
			...
			// for example H2 treat both cases as false - however the spec and all major
			// vendors support the following:
	...
	private CompareCriteria simplifyMathematicalCriteria(CompareCriteria criteria) throws TeiidProcessingException {
		...
		FunctionDescriptor descriptor = funcLib.findFunction(oppFunc,
				new Class[] { rightExpr.getType(), const1.getType() });
		if (descriptor == null) {
			// See defect 9380 - this can be caused by const2 being a null Constant, for
			// example (? + 1) < null
			return criteria;
		}
		...
		if (rightExpr instanceof Constant) {
			...
			try {
				Object result = descriptor.invokeFunction(new Object[] { const2.getValue(), const1.getValue() }, null,
						this.context);
	 ...
	 * into
	 * 
	 * <code>typedColumn = convert('5', typeOfColumn)</code> where 'typeOfColumn' is
	 * the type of 'typedColumn'
	 * 
	 * if, for example, the type of the column is integer, than the above can be
	 * pre-evaluated to
	 * 
	 * <code>typedColumn = 5 </code>
	 * 
	 * Right expression has already been checked to be a Constant, left expression
	 * to be a function. Function is known to be "convert" or "cast".
	 * 
	 * @param crit
	 *            CompareCriteria
	...
	private Criteria simplifyConvertFunction(CompareCriteria crit) {
		...
		Constant result = ResolverUtil.convertConstant(DataTypeManager.getDataTypeName(rightConstant.getType()),
	 ...
	 * 
	 * Right expression has already been checked to be a Constant, left expression
	 * to be a function. Function is known to be "convert" or "cast". The scope of
	 * this change will be limited to the case where the left expression is
	 ...
	 * @since 4.2
	 */
	private Criteria simplifyConvertFunction(SetCriteria crit)
			throws TeiidComponentException, TeiidProcessingException {
	...
	private Criteria simplifyParseFormatFunction(CompareCriteria crit) {
		...
		FunctionLibrary funcLib = this.metadata.getFunctionLibrary();
		FunctionDescriptor descriptor = funcLib.findFunction(inverseFunction,
				new Class[] { rightExpr.getType(), formatExpr.getType() });
		if (descriptor == null) {
			return crit;
		}
		Object value = ((Constant) rightExpr).getValue();
		try {
			Object result = descriptor.invokeFunction(
					new Object[] { context, ((Constant) rightExpr).getValue(), format }, null, this.context);
			result = leftFunction.getFunctionDescriptor().invokeFunction(new Object[] { context, result, format }, null,
					this.context);
			if (Constant.COMPARATOR.compare(value, result) != 0) {
				return getSimpliedCriteria(crit, leftExpr, crit.getOperator() != CompareCriteria.EQ, true);
			}
		} catch (FunctionExecutionException e) {
			// Not all numeric formats are invertable, so just return the criteria as it may
			// still be valid
			return crit;
		} catch (BlockedException e) {
			return crit;
	...
	/**
	 * This method applies a similar simplification as the previous method for Case
	 * 1829. This is conceptually the same thing but done using the timestampCreate
	 * system function.
	 * 
	 * TIMESTAMPCREATE(rpcolli_physical.RPCOLLI.Table_B.date_field,
	 * rpcolli_physical.RPCOLLI.Table_B.time_field) = {ts'1969-09-20 18:30:45.0'}
	 * 
	 * -------------
	 * 
	 * rpcolli_physical.RPCOLLI.Table_B.date_field = {d'1969-09-20'} AND
	...
	private Criteria simplifyTimestampMerge2(CompareCriteria criteria) {
		...
		CompareCriteria dateCrit = new CompareCriteria(args[0], CompareCriteria.EQ,
				new Constant(date, DataTypeManager.DefaultDataClasses.DATE));
		CompareCriteria timeCrit = new CompareCriteria(args[1], CompareCriteria.EQ,
				new Constant(time, DataTypeManager.DefaultDataClasses.TIME));
	 ...
	 * conceptually the same thing but done using the timestampCreate system
	 * function.
	 * 
	...
	private Criteria simplifyTimestampMerge(CompareCriteria criteria) {
		...
		if (!(formatDateFunction.getName().equalsIgnoreCase("formatdate") //$NON-NLS-1$
				&& formatTimeFunction.getName().equalsIgnoreCase("formattime"))) { //$NON-NLS-1$
			return criteria;
		}

		// Verify format functions have constants
		if (!(formatDateFunction.getArgs()[1] instanceof Constant
				&& formatTimeFunction.getArgs()[1] instanceof Constant)) {
			return criteria;
		}
		...
		try {
			...
			CompareCriteria dateCompare = new CompareCriteria(formatDateFunction.getArgs()[0], CompareCriteria.EQ,
					dateConstant);

			Constant timeConstant = new Constant(TimestampWithTimezone.createTime(ts));
			CompareCriteria timeCompare = new CompareCriteria(formatTimeFunction.getArgs()[0], CompareCriteria.EQ,
					timeConstant);
		...
		if (rightExpr instanceof Constant && rightExpr.getType().equals(DataTypeManager.DefaultDataClasses.STRING)) {
			...
			if (criteria.getMode() != MatchMode.REGEX) {
				...
				if (criteria.getMode() == MatchMode.SIMILAR) {
					...
					criteria.setRightExpression(
							new Constant(Evaluator.SIMILAR_TO_REGEX.getPatternString(value, escape)));
					criteria.setEscapeChar(MatchCriteria.NULL_ESCAPE_CHAR);
				} else if (DataTypeManager.DefaultDataClasses.STRING.equals(criteria.getLeftExpression().getType())
						&& value.indexOf(escape) < 0 && value.indexOf(MatchCriteria.MATCH_CHAR) < 0
						&& value.indexOf(MatchCriteria.WILDCARD_CHAR) < 0) {
					// if both left and right expressions are strings, and the LIKE match characters
					// ('*', '_') are not present
	...
	private boolean rewriteLeftExpression(AbstractSetCriteria criteria)
			throws TeiidComponentException, TeiidProcessingException {
	...
	private Criteria rewriteCriteria(SetCriteria criteria) throws TeiidComponentException, TeiidProcessingException {
		if (criteria.isAllConstants() && criteria.getValues().size() > 1
				&& criteria.getExpression() instanceof ElementSymbol) {
			return criteria;
		}
	...
	public static Expression rewriteExpression(Expression expression, CommandContext context,
	...
	public static Expression rewriteExpression(Expression expression, CommandContext context,
			QueryMetadataInterface metadata, boolean rewriteSubcommands)
			throws TeiidComponentException, TeiidProcessingException {
		QueryRewriter rewriter = new QueryRewriter(metadata, context);
	...
	private Expression rewriteExpressionDirect(Expression expression)
			throws TeiidComponentException, TeiidProcessingException {
		...
		if (expression instanceof ElementSymbol) {
			...
			if (!processing && es.isExternalReference()) {
				...
				if (value == null) {
					...
					try {
						return new Constant(FunctionMethods.convert(((Constant) value).getValue(),
								DataTypeManager.getDataTypeName(type)), es.getType());
					} catch (FunctionExecutionException e) {
						throw new QueryValidatorException(e);
					}
				}
	...
	private Expression rewriteFunction(Function function) throws TeiidComponentException, TeiidProcessingException {
		...
		if (StringUtil.startsWithIgnoreCase(functionName, "parse")) { //$NON-NLS-1$
			...
			if (PARSE_FORMAT_TYPES.contains(type) && Number.class.isAssignableFrom(function.getType())
				...
				FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.PARSEBIGDECIMAL,
				...
				return rewriteFunction(ResolverUtil.getConversion(result, DataTypeManager.DefaultDataTypes.BIG_DECIMAL,
						DataTypeManager.getDataTypeName(function.getType()), false, metadata.getFunctionLibrary()));
			} else if ((DataTypeManager.DefaultDataTypes.DATE.equalsIgnoreCase(type)
					|| DataTypeManager.DefaultDataTypes.TIME.equalsIgnoreCase(type))
					&& function.getArg(1) instanceof Constant) {
				...
				if (format.equals(c.getValue())) {
					...
					if ((arg instanceof Function) && FunctionLibrary.isConvert((Function) arg)
							&& java.util.Date.class.isAssignableFrom(((Function) arg).getArg(0).getType())) {
						return rewriteExpressionDirect(ResolverUtil.getConversion(arg,
								DataTypeManager.DefaultDataTypes.STRING, type, false, metadata.getFunctionLibrary()));
					}
				}
			}
		} else if (StringUtil.startsWithIgnoreCase(functionName, "format")) { //$NON-NLS-1$
			String type = functionName.substring(6);
			if (PARSE_FORMAT_TYPES.contains(type) && Number.class.isAssignableFrom(function.getArg(0).getType())
					&& !type.equals(DataTypeManager.DefaultDataTypes.BIG_DECIMAL)) {
				...
				Function result = new Function(SourceSystemFunctions.FORMATBIGDECIMAL,
						new Expression[] { bigDecimalParam, function.getArg(1) });
				FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.FORMATBIGDECIMAL,
						new Class[] { DataTypeManager.DefaultDataClasses.BIG_DECIMAL,
								DataTypeManager.DefaultDataClasses.STRING });
				result.setFunctionDescriptor(descriptor);
				...
				Constant c = (Constant) function.getArg(1);
				if (format.equals(c.getValue())) {
					return rewriteExpressionDirect(ResolverUtil.getConversion(function.getArg(0),
							DataTypeManager.getDataTypeName(function.getArg(0).getType()),
							DataTypeManager.DefaultDataTypes.STRING, false, metadata.getFunctionLibrary()));
				}
			}
		}
		...
		if (code != null) {
			...
			case 0: { // space(x) => repeat(' ', x)
				...
				FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.REPEAT, new Class[] {
			...
			case 2: { // rewrite nullif(a, b) => case when (a = b) then null else a
				List when = Arrays.asList(new Criteria[] {
						new CompareCriteria(function.getArg(0), CompareCriteria.EQ, function.getArg(1)) });
			...
			case 3: {
				...
				if (args.length == 2) {
					...
					// resolve the function
					FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.IFNULL,
			...
			case 5: {
				if (function.getType() != DataTypeManager.DefaultDataClasses.TIMESTAMP) {
					FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.TIMESTAMPADD,
							new Class[] { DataTypeManager.DefaultDataClasses.STRING,
									DataTypeManager.DefaultDataClasses.INTEGER,
									DataTypeManager.DefaultDataClasses.TIMESTAMP });
					function.setFunctionDescriptor(descriptor);
					...
					function.getArgs()[2] = ResolverUtil.getConversion(function.getArg(2),
							DataTypeManager.getDataTypeName(type), DataTypeManager.DefaultDataTypes.TIMESTAMP, false,
							funcLibrary);
					function = ResolverUtil.getConversion(function, DataTypeManager.DefaultDataTypes.TIMESTAMP,
							DataTypeManager.getDataTypeName(type), false, funcLibrary);
				}
				break;
			}
			case 6:
			case 7: {
				FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.PARSETIMESTAMP,
						new Class[] { DataTypeManager.DefaultDataClasses.STRING,
								DataTypeManager.DefaultDataClasses.STRING });
			...
			case 8:
			case 9: {
				FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.FORMATTIMESTAMP,
			...
			case 10: {
				if (new Constant(" ").equals(function.getArg(1))) { //$NON-NLS-1$
					...
					if (!SQLConstants.Reserved.TRAILING.equalsIgnoreCase(spec)) {
						...
						FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.LTRIM,
								new Class[] { DataTypeManager.DefaultDataClasses.STRING });
						function.setFunctionDescriptor(descriptor);
					...
					if (!SQLConstants.Reserved.LEADING.equalsIgnoreCase(spec)) {
						...
						FunctionDescriptor descriptor = funcLibrary.findFunction(SourceSystemFunctions.RTRIM,
								new Class[] { DataTypeManager.DefaultDataClasses.STRING });
						function.setFunctionDescriptor(descriptor);
						function.setType(DataTypeManager.DefaultDataClasses.STRING);
					}
		...
		if (FunctionLibrary.isConvert(function)) {
			...
			if (tgtType == DataTypeManager.DefaultDataClasses.STRING
					&& (nestedType == DataTypeManager.DefaultDataClasses.BOOLEAN
							...
							|| (tgtType == DataTypeManager.DefaultDataClasses.DOUBLE
									&& srcType != DataTypeManager.DefaultDataClasses.FLOAT))) {
				return function;
			}
			// nested implicit transform is not needed
			return rewriteExpressionDirect(
					ResolverUtil.getConversion(nested.getArgs()[0], DataTypeManager.getDataTypeName(nestedType),
							DataTypeManager.getDataTypeName(tgtType), false, funcLibrary));
		}
	...
	private Expression convertDecodeFunction(Function function) {
		...
		while (tokenizer.hasMoreTokens()) {
			String resultString;
			String compareString = convertString(tokenizer.nextToken().trim());
			if (tokenizer.hasMoreTokens()) {
				...
				if (compareString == null) {
					...
					crit = new CompareCriteria((Expression) exprs[0].clone(), CompareCriteria.EQ,
							new Constant(compareString));
				}
				newWhens.add(crit);
		...
		if (function.getName().equalsIgnoreCase(FunctionLibrary.DECODEINTEGER)) {
			return ResolverUtil.getConversion(newCaseExpr, DataTypeManager.DefaultDataTypes.STRING,
	...
	private static String convertString(String string) {
		/*
		 * if there are no characters in the compare string we designate that as an
		 * indication of null. ie if the decode string looks like this:
		 *
		 * "'this', 1,,'null'"
		 *
		...
		/*
		 * we also allow the use of the keyword null in the decode string. if it wished
		 * to match on the string 'null' then the string must be qualified by '
		 * designators.
		 */
		...
		/*
		 * Here we check to see if the String in the decode String submitted was
		 * surrounded by String literal characters. In this case we strip these literal
		...
		if ((string.startsWith("\"") && string.endsWith("\"")) //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (string.length() == 2) {
				/*
				 * This is an indication that the desired string to be compared is the "" empty
				 * string, so we return it as such.
				 */
				string = ""; //$NON-NLS-1$
	...
	private Expression rewriteCaseExpression(SearchedCaseExpression expr)
		...
		/*
		 * optimization for case 5413: If all of the remaining 'thens' and the 'else'
		 * evaluate to the same value, just return the 'else' expression.
		 */
	...
	private Command rewriteExec(StoredProcedure storedProcedure)
			throws TeiidComponentException, TeiidProcessingException {
	...
	private Command rewriteInsert(Insert insert) throws TeiidComponentException, TeiidProcessingException {
		if (insert.isMerge()) {
			...
			if (keys.isEmpty()) {
				throw new QueryValidatorException(QueryPlugin.Event.TEIID31132,
						QueryPlugin.Util.gs(QueryPlugin.Event.TEIID31132, insert.getGroup()));
			}
		...
		if (info != null && info.isInherentInsert()) {
			...
			if (mapping == null) {
				throw new QueryValidatorException(QueryPlugin.Event.TEIID30375,
						QueryPlugin.Util.gs(QueryPlugin.Event.TEIID30375, insert.getVariables()));
			}
			...
			insert.setUpdateInfo(
					ProcedureContainerResolver.getUpdateInfo(insert.getGroup(), metadata, Command.TYPE_INSERT, true));
	...
	public static Query createInlineViewQuery(GroupSymbol inlineGroup, Command nested, QueryMetadataInterface metadata,
		...
		List<Expression> selectSymbols = SetQuery
				.getTypedProjectedSymbols(ResolverUtil.resolveElementsInGroup(inlineGroup, tma), actualTypes, tma);
	...
	private Command rewriteInherentUpdate(Update update, UpdateInfo info) throws QueryValidatorException,
			QueryMetadataException, TeiidComponentException, QueryResolverException, TeiidProcessingException {
		UpdateMapping mapping = info.findUpdateMapping(update.getChangeList().getClauseMap().keySet(), false);
		if (mapping == null) {
			throw new QueryValidatorException(QueryPlugin.Event.TEIID30376,
					QueryPlugin.Util.gs(QueryPlugin.Event.TEIID30376, update.getChangeList().getClauseMap().keySet()));
		}
		Map<ElementSymbol, ElementSymbol> symbolMap = mapping.getUpdatableViewSymbols();
		if (info.isSimple()) {
			...
			if (fullMapping != null) {
				...
				update.setUpdateInfo(ProcedureContainerResolver.getUpdateInfo(update.getGroup(), metadata,
						Command.TYPE_UPDATE, true));
				return rewriteUpdate(update);
			}
		}
		Query query = (Query) info.getViewDefinition().clone();
		query.setOrderBy(null);
		SymbolMap expressionMapping = SymbolMap.createSymbolMap(update.getGroup(), query.getProjectedSymbols(),
				metadata);
	...
	private ArrayList<Expression> mapChangeList(SetClauseList setClauses, Map<ElementSymbol, ElementSymbol> symbolMap,
			GroupSymbol varGroup) {
	...
	private static Expression mapExpression(GroupSymbol varGroup, ArrayList<Expression> selectSymbols, int i,
			Expression ex) {
	...
	private Command createUpdateProcedure(Update update, Query query, GroupSymbol group, String correlationName,
			SetClauseList setClauseList, GroupSymbol varGroup, Criteria constraint)
			throws TeiidComponentException, QueryMetadataException, QueryResolverException, TeiidProcessingException {
	...
	/**
	 * rewrite as loop on (query) as X begin newupdate; rows_updated = rows_updated
	 * + 1 end;
	 * 
	 * @param updateType
	 */
	private Command asLoopProcedure(GroupSymbol group, QueryCommand query, ProcedureContainer newUpdate,
			GroupSymbol varGroup, int updateType)
			throws QueryResolverException, TeiidComponentException, TeiidProcessingException {
		return asLoopProcedure(group, query, new CommandStatement(newUpdate), varGroup, updateType);
	}

	private Command asLoopProcedure(GroupSymbol group, QueryCommand query, Statement s, GroupSymbol varGroup,
			int updateType) throws QueryResolverException, TeiidComponentException, TeiidProcessingException {
		Block b = new Block();
		b.addStatement(s);
		...
		ElementSymbol rowsUpdated = new ElementSymbol(
				ProcedureReservedWords.VARIABLES + Symbol.SEPARATOR + "ROWS_UPDATED"); //$NON-NLS-1$
	...
	private List<Criteria> createPkCriteria(GroupSymbol group, String correlationName, Query query,
			GroupSymbol varGroup) throws TeiidComponentException, QueryMetadataException {
	...
	private Command rewriteInherentDelete(Delete delete, UpdateInfo info)
		...
		if (info.isSimple()) {
			...
			if (fullMapping != null) {
				...
				// TODO: properly handle correlated references
				DeepPostOrderNavigator.doVisit(delete,
						new ExpressionMappingVisitor(mapping.getUpdatableViewSymbols(), true));
				delete.setUpdateInfo(ProcedureContainerResolver.getUpdateInfo(delete.getGroup(), metadata,
						Command.TYPE_DELETE, true));
				if (info.getViewDefinition().getCriteria() != null) {
		...
		Query query = (Query) info.getViewDefinition().clone();
		query.setOrderBy(null);
		SymbolMap expressionMapping = SymbolMap.createSymbolMap(delete.getGroup(), query.getProjectedSymbols(),
				metadata);
	...
	public static Command createDeleteProcedure(Delete delete, QueryMetadataInterface metadata, CommandContext context)
			throws QueryResolverException, QueryMetadataException, TeiidComponentException, TeiidProcessingException {
		QueryRewriter rewriter = new QueryRewriter(metadata, context);
		Criteria crit = delete.getCriteria();
		Query query = new Query(new Select(), new From(Arrays.asList(new UnaryFromClause(delete.getGroup()))), crit,
				null, null);
		return rewriter.createDeleteProcedure(delete, query, delete.getGroup(), delete.getGroup().getName());
	}

	public static Command createUpdateProcedure(Update update, QueryMetadataInterface metadata, CommandContext context)
			throws QueryResolverException, QueryMetadataException, TeiidComponentException, TeiidProcessingException {
		QueryRewriter rewriter = new QueryRewriter(metadata, context);
		Criteria crit = update.getCriteria();
		if (crit != null) {
		...
		Query query = new Query(new Select(selectSymbols),
				new From(Arrays.asList(new UnaryFromClause(update.getGroup()))), crit, null, null);
		return rewriter.createUpdateProcedure(update, query, update.getGroup(), update.getGroup().getName(),
				setClauseList, varGroup, constraint);
	}

	private Command createDeleteProcedure(Delete delete, Query query, GroupSymbol group, String correlationName)
			throws TeiidComponentException, QueryMetadataException, QueryResolverException, TeiidProcessingException {
