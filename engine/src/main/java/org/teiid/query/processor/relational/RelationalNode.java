        ...
        try {
            while (true) {
                ...
                if (recordStats) {
                    ...
                    this.getProcessingState().nodeStatistics.collectCumulativeNodeStats(batch.getRowCount(), RelationalNodeStatistics.BATCHCOMPLETE_STOP);
        ...
        try {
            ...
            if (recordStats) {
                ...
                if (LogManager.isMessageToBeRecorded(org.teiid.logging.LogConstants.CTX_DQP, MessageLevel.TRACE) && !buffer.isForwardOnly()) {
                	for (int i = 1; i <= buffer.getRowCount(); i+=buffer.getBatchSize()) {
