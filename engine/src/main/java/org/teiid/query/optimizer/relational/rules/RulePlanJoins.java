...
public class RulePlanJoins implements OptimizerRule {
    ...
    public PlanNode execute(PlanNode plan,
        ...
        planLeftOuterJoinAssociativity(plan, metadata, capabilitiesFinder,
    ...
    /*
     * TODO: this is not yet done deeply.  It runs at a single level and then rule raise access runs.
	 * if there are multiple are several stages of such rearrangements, then we should consider breaking 
	 * this off as it's own rule after rule raise access that performs raise and keeps running until the
	 * plan is stable
     */
	private void planLeftOuterJoinAssociativity(PlanNode plan,
			QueryMetadataInterface metadata,
			CapabilitiesFinder capabilitiesFinder,
    	...
    	while (!joins.isEmpty()) {
    		...
    		if (isLeftOuterJoin(left)) {
    			childJoin = left;
    			other = right;
    		} else if (isLeftOuterJoin(right)) {
    			childJoin = right;
    			other = left;
    		} else {
    			continue;
    		...
    		PlanNode cSource = FrameUtil.findJoinSourceNode(other);
			...
			if (Collections.disjoint(groups, FrameUtil.findJoinSourceNode(childJoin == left?childJoin.getFirstChild():childJoin.getLastChild()).getGroups())) {
				...
				PlanNode bSource = FrameUtil.findJoinSourceNode(childJoin == left?childJoin.getLastChild():childJoin.getFirstChild());
				if (bSource.getType() != NodeConstants.Types.ACCESS) {
    				continue;
    			}
    			if (RuleRaiseAccess.canRaiseOverJoin(childJoin == left?Arrays.asList(bSource, cSource):Arrays.asList(cSource, bSource), metadata, capabilitiesFinder, joinCriteria, JoinType.JOIN_LEFT_OUTER, analysisRecord, context, false) == null) {
    				continue;
    			}
    			//rearrange
    			PlanNode newParent = createJoinNode();
    			newParent.setProperty(Info.JOIN_TYPE, JoinType.JOIN_LEFT_OUTER);
    			PlanNode newChild = createJoinNode();
				...
				PlanNode aSource = FrameUtil.findJoinSourceNode(childJoin == left?childJoin.getFirstChild():childJoin.getLastChild());
    			...
    			if (RuleRaiseAccess.canRaiseOverJoin(childJoin == left?Arrays.asList(aSource, cSource):Arrays.asList(cSource, aSource), metadata, capabilitiesFinder, joinCriteria, JoinType.JOIN_LEFT_OUTER, analysisRecord, context, false) == null) {
    				continue;
    			}
    ...
    private boolean isLeftOuterJoin(PlanNode left) {
    	return left.getType() == NodeConstants.Types.JOIN && left.getProperty(Info.JOIN_TYPE) == JoinType.JOIN_LEFT_OUTER;
    }
        ...
        for (Iterator accessNodeIter = accessMap.entrySet().iterator(); accessNodeIter.hasNext();) {
            ...
            for (int i = accessNodes.size() - 1; i >= 0; i--) {
                ...
                for (int k = (secondPass==-1?accessNodes.size() - 1:secondPass); k >= 0; k--) {
                    ...
                    if (!shouldPush || RuleRaiseAccess.canRaiseOverJoin(toTest, metadata, capFinder, joinCriteria, joinType, null, context, secondPass != -1) == null) {
