...
public class JsonPathFunctionMethods {
    ...
    public static Object[] jsonToArray(Clob clob, String jsonPath, boolean nullLeaf, String... colpaths)
        ...
        for (int i = 0; i < colpaths.length; i++) {
            ...
            if (path == null || !path.trim().startsWith("@")) { //$NON-NLS-1$
        ...
        if (result instanceof JSONArray) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                rows.add(buildRow(value, conf, paths));
            } 
        } else {
            buildRow(result, conf, paths);
        }
        
        return rows.toArray(new Object[rows.size()]);
    }
    
    private static Object[] buildRow(Object value, Configuration conf, JsonPath[] paths) {
