...
public final class ClobType implements Streamable, Clob, Sequencable {
    ...
    public ClobType(Clob clob) {
        if (clob == null) {
            throw new IllegalArgumentException(CommonPlugin.Util.getString("ClobValue.isNUll")); //$NON-NLS-1$
        }
    ...
    private void checkReference() {
        if (this.srcClob == null) {
            throw new InvalidReferenceException(CommonPlugin.Util.getString("ClobValue.InvalidReference")); //$NON-NLS-1$
        }
    }     
