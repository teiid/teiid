...
public class BlobImpl implements Blob, Serializable {
    ...
    public byte[] getBytes(long pos, int length) throws SQLException {
        if (pos < 1) {
            throw new SQLException(CommonPlugin.Util.getString("BlobImpl.Invalid_byte_position",  new Long(pos))); //$NON-NLS-1$
        }
        ...
        if (length < 0) {
            throw new SQLException(CommonPlugin.Util.getString("BlobImpl.Invalid_bytes_length",  new Long(length))); //$NON-NLS-1$
        }
    ...
    public long position(byte[] pattern, long start) throws SQLException {
        if (start < 1) {
            throw new SQLException(CommonPlugin.Util.getString("BlobImpl.Invalid_start_position",  new Long(start))); //$NON-NLS-1$
        }
        if (pattern == null || start > data.length) {
            return -1;
        }
