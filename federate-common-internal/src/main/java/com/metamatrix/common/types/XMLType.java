...
public class XMLType implements Streamable, SQLXML {
    ...
    public XMLType(SQLXML xml) {      
        if (xml == null) {
            throw new IllegalArgumentException(CommonPlugin.Util.getString("XMLValue.isNUll")); //$NON-NLS-1$
        }
    ...
    private void checkReference() {
        if (this.srcXML == null) {
            throw new InvalidReferenceException(CommonPlugin.Util.getString("XMLValue.InvalidReference")); //$NON-NLS-1$
        }
    }
