...
public class ClobImpl implements Clob, Serializable {
    ...
    public ClobImpl(Clob clob) throws SQLException {
        try {
            ...
            throw new SQLException(CommonPlugin.Util.getString("ClobImpl.Failed_copy_clob",  ioe.getMessage())); //$NON-NLS-1$
        }
    }
    ...
    public String getSubString(long pos, int length) throws SQLException {
        ...
        if (pos < 1) {
            throw new SQLException(CommonPlugin.Util.getString("ClobImpl.Invalid_substring_position",  new Long(pos))); //$NON-NLS-1$
        }
        ...
        if (length < 0) {
            throw new SQLException(CommonPlugin.Util.getString("ClobImpl.Invalid_substring_length",  new Long(length))); //$NON-NLS-1$
        }
    ...
    */
    public long position(String searchstr, long start) throws SQLException {
        if (start < 1) {
            throw new SQLException(CommonPlugin.Util.getString("ClobImpl.Invalid_start_position",  new Long(start))); //$NON-NLS-1$
        }
        if (searchstr == null || start > data.length) {
            return -1;
        }
