...
public class TestMMDatabaseMetaData {
    ...
    private void initResultSetStreams(String testName) throws FileNotFoundException {
        if (REPLACE_EXPECTED) {
            File actual = new File(UnitTestUtil.getTestDataPath() + "/TestDatabaseMetaDataImpl/"+testName+".expected"); //$NON-NLS-1$ //$NON-NLS-2$
            actualOut = new FileOutputStream(actual);
        } else {
            if (WRITE_ACTUAL_RESULTS_TO_FILE) {
                File actual = new File(UnitTestUtil.getTestDataPath() + "/TestDatabaseMetaDataImpl/"+testName+".actual"); //$NON-NLS-1$ //$NON-NLS-2$
                actualOut = new FileOutputStream(actual);
            }
            File expected = new File(UnitTestUtil.getTestDataPath() + "/TestDatabaseMetaDataImpl/"+testName+".expected"); //$NON-NLS-1$ //$NON-NLS-2$
