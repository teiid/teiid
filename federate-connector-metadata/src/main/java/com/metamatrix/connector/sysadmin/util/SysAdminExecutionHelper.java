                        ...
                        if(value != null) {
                            ...
                            if (javaType == Types.JAVA_OBJECT) {
                                ...
                                if (value instanceof Collection || value instanceof List) {
                                    ...
                                    transform = determineTransformation(valueType, valueType, resultsTranslator.getValueTranslators());
...
//                                    else if (value instanceof Collection) {
//                                        List l = new ArrayList((Collection) value);
//                                        batch.addRow(l);
//                                    }  
...
//                                value = translateOneValue(value, command, valueType, columnTypes[0], resultsTranslator.getValueTranslators(), context, dbmsTimeZone, trimStrings);
                                // if the result is a primitive, non-java object, then there should only be
                                // one column type
                                transform = determineTransformation(valueType, columnTypes[0], resultsTranslator.getValueTranslators());
...
// System.out.println("\nRead value = " + value + " of type " + (value != null ? value.getClass().getName() : ""));
    ...
    //                            if(trimColumn[i]) {
            ...
            throw new ConnectorException(
                e,
                SysAdminPlugin.Util.getString("ObjectExecutionHelper.Unknown_error_translating_results___9", e.getMessage())); //$NON-NLS-1$
        }
    }  
                        ...
                        if(value != null) {
                            ...
                            if(! transformKnown[i]) {
                                ...
                                if(valueType != columnTypes[i]) {                                    
                                    transforms[i] = determineTransformation(valueType, columnTypes[i], resultsTranslator.getValueTranslators());
                                }
...
// System.out.println("\nRead value = " + value + " of type " + (value != null ? value.getClass().getName() : ""));
                            ...
                            if(transforms[i] != null) {
                                value = transforms[i].translate(value, command, context);
                            }
...
//                }
                ...
                // 1st check is the most restrictive check to find the translator that exactly matches
//                if(expectedType.equals(translator.getTargetType()) && actualType.equals(translator.getSourceType()) ) {
                    valueTranslator = translator;
                    break;
                    
                // 2nd check - target must match, the source can be an implementation of the actual type
                } 
                ...
                if (assignableTarget && assignableSource) {
                    valueTranslator = translator;
                    break;
                }                   
//                    
...
//                    break;
                 ...
                 // 3rd check - source must match, the target can be an implementation of the expected type                    
//                } else if(actualType.equals(translator.getSourceType()) && translator.getTargetType().isAssignableFrom(expectedType)){
//                    valueTranslator = translator;
//                    break;
                 
                // 4th check - least restrictive, both tarter and source can be assignable
                // This is 
//                } else if( translator.getTargetType().isAssignableFrom(expectedType) && translator.getSourceType().isAssignableFrom(actualType)) {
//                    valueTranslator = translator;
//                    break;
                    
 //                  expectedType.equals(translator.getTargetType()) && translator.getSourceType().isAssignableFrom(actualType)){
//                }
        ...
        if(valueTranslator == null){
            ...
            if(transform == null) {
                throw new ConnectorException(SysAdminPlugin.Util.getString("ExecutionHelper.Unable_to_translate_data_value__1", actualType.getName(), expectedType.getName())); //$NON-NLS-1$
            }
