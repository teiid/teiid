...
public class LiteralParser {
	...
	private static final Pattern DATETIME_PATTERN = Pattern.compile("" +
		...
		"((Z)|([\\+|-]?\\d{2}:\\d{2}))?" + // group 4 (tz, ignored - handles bad services)
		"$");
	...
	private static Timestamp parseDateTime(String value) throws ParseException {
		...
		if (offset == null) {
			return new Timestamp(adjustMillis(DATETIME_WITH_MILLIS_FORMAT.parse(dateTime + seconds+ nanoSeconds.substring(0, 4)).getTime(), nanoSeconds));
		}
		return new Timestamp(adjustMillis(DATETIME_WITH_MILLIS_FORMAT.parse(dateTime + seconds+ nanoSeconds.substring(0, 4)+offset).getTime(), nanoSeconds));
	}	
