...
public class ParquetMetadataProcessor implements MetadataProcessor<VirtualFileConnection> {
    ...
    @ExtensionMetadataProperty(applicable= Table.class, datatype=String.class, display="Partionong Scheme Name", description="Partitioning Scheme Name, used to describe the partitioning scheme the parquet files follow", required=true)
    public static final String PARTITIONING_SCHEME = MetadataFactory.PARQUET_PREFIX+"PARTITIONING_SCHEME"; //$NON-NLS-1$

    @ExtensionMetadataProperty(applicable= Table.class, datatype=String.class, display="Partitioned Columns", description="Partitioning Scheme Name, used to describe the partitioning scheme the parquet files follow", required=true)
