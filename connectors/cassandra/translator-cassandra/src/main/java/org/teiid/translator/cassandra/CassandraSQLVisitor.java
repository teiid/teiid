...
public class CassandraSQLVisitor extends SQLStringVisitor {
	...
	public void visit(Select obj) {
		...
		LogManager.logTrace(LogConstants.CTX_RUNTIME, "sql2cql: " + buffer); //$NON-NLS-1$
	}
	...
	public void visit(Literal obj) {
		...
		/*
		 * if (obj.isBindEligible() || obj.getType() ==
		 * TypeFacility.RUNTIME_TYPES.OBJECT || type ==
		 * TypeFacility.RUNTIME_TYPES.VARBINARY) { if (values == null) { values
		 * = new ArrayList<Object>(); } buffer.append('?'); if (type ==
		 * TypeFacility.RUNTIME_TYPES.VARBINARY) {
		 * values.add(ByteBuffer.wrap(((BinaryType)obj.getValue()).
		 * getBytesDirect())); } else { values.add(obj.getValue()); } return; }
		 */
		...
		if (!Number.class.isAssignableFrom(type) && type != TypeFacility.RUNTIME_TYPES.BOOLEAN
