...
Before executing the run script, first we need deploy 'stateService' to JBoss Server, the 'stateService' is a JAX-WS compatible web service, which supply service for GetAllStateInfo and GetStateInfo via StateCode. 

Build 'stateService' via:
	cd stateService/
	mvn clean install
